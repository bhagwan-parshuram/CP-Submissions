...
//ll c(int n,int m){if(m>n)return 0;ll fac[n+1];fac[0]=1;Fo(i,1,n+1)fac[i]=(i*fac[i-1])%mod;return ((fac[n]*inv(fac[m])%mod)*inv(fac[n-m]))%mod;}
//=======================
...
void solve() {
  ...
  Fo(i,1,A+1)ans-=c(f[i],2);
  Fo(i,1,B+1)ans-=c(s[i],2);
