...
void solve() {
    ...
    //       ans += odd[i];
    // }
    ll curr = odd[0]+even[0];
    int mini = min(odd.size(), even.size());
    for (int i = 1; i < mini; i++)
    {
        em = min(em, even[i]);
        ans = min(ans, curr + (n - i) * om + (n - i - 1) * em+even[i]);
        om = min(odd[i], om);
        curr += odd[i] + even[i];
        ans = min(ans, curr + (n - i-1) * (em+ om));
    }
    if (odd.size() < even.size()){
       // debug(curr);
        int x = odd.size();
        em = min(em, even[x]);
        ans = min(ans, curr + even[x] + (n - x - 1) * em + (n - x) * om);
    }
    cout << ans << endl;
}
